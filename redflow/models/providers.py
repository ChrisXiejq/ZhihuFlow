...
class OpenAICompatibleModel:
    ...
    def from_env(cls) -> Optional["OpenAICompatibleModel"]:
        api_key = os.getenv("REDFLOW_OPENAI_API_KEY")
        base_url = os.getenv("REDFLOW_OPENAI_BASE_URL", "https://api.openai.com/v1")
        model = os.getenv("REDFLOW_OPENAI_MODEL", "gpt-4o-mini")
        if not api_key:
            return None
...
class BailianModel(OpenAICompatibleModel):
    ...
    def from_env(cls) -> Optional["BailianModel"]:
        ...
        model = os.getenv("REDFLOW_BAILIAN_MODEL") or os.getenv("SOLOOPS_MODEL_NAME") or "qwen-plus"
        base_url = os.getenv("REDFLOW_BAILIAN_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")
        if not api_key:
            return None
...
def model_from_env(provider: Optional[str] = None) -> ModelProvider:
    requested = (provider or os.getenv("REDFLOW_MODEL_PROVIDER") or os.getenv("SOLOOPS_MODEL_PROVIDER") or "").lower()
