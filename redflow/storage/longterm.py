...
from redflow.core.schemas import new_id, utc_now
...
class LongTermMemory:
    ...
    def __init__(self, path: str = ".redflow/memory.json") -> None:
        self.path = Path(path)
    ...
    def remember_run(self, topic: str, trace_id: str, policy_risk: str, source_count: int) -> None:
        ...
        self._upsert_fact(
            data,
            MemoryFact(
                text="User is building RedFlow as both a real Zhihu GMV content system and a job-search portfolio project.",
                tags=["user_goal", "redflow", "portfolio"],
                confidence=0.92,
            ),
        )
        self._write(data)
