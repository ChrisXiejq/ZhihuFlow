...
class EmailDelivery:
    ...
    def __init__(
        ...
        self.username = username or os.getenv("ZHIHUFLOW_SMTP_USER")
        self.password = password or os.getenv("ZHIHUFLOW_SMTP_PASSWORD")
        self.sender = sender or os.getenv("ZHIHUFLOW_EMAIL_FROM") or self.username
        self.recipient = recipient or os.getenv("ZHIHUFLOW_EMAIL_TO")
        self.host = host or os.getenv("ZHIHUFLOW_SMTP_HOST") or infer_smtp_host(self.sender or "")
        self.port = int(port or os.getenv("ZHIHUFLOW_SMTP_PORT", "465"))
    ...
    def send_article(self, subject: str, markdown: str, summary_json: str, *, dry_run: bool = False) -> DeliveryResult:
        ...
        msg.set_content(
            "ZhihuFlow 已生成一篇新的知乎文章草稿。\n\n"
            "正文 Markdown 和结构化运行摘要已作为附件发送。\n"
            "请人工审核后再发布。\n"
        )
        msg.add_attachment(markdown, subtype="markdown", filename="zhihuflow_article.md")
        msg.add_attachment(summary_json, subtype="json", filename="zhihuflow_run.json")
